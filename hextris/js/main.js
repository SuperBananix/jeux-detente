...
function checkGameOver() {
	for (var i = 0; i < MainHex.sides; i++) {
		if (isInfringing(MainHex)) {
			// RevisionApp (24/09/2026) : envoi du score à un serveur externe retiré.
...
function showHelp() {
	...
	$("#inst_main_body").html("<div id = 'instructions_head'>COMMENT JOUER</div><p>Le but : empêcher les blocs de sortir de l'hexagone gris.</p><p>" + (settings.platform != 'mobile' ? 'Appuie sur les flèches gauche et droite' : 'Touche le côté gauche ou droit de l\'écran') + " pour faire tourner l'hexagone." + (settings.platform != 'mobile' ? ' La flèche du bas fait tomber les blocs plus vite.' : '') + "</p><p>Assemble au moins 3 blocs de la même couleur pour les faire disparaître et gagner des points.</p><p>Les <span style='color:#f1c40f;'>lignes</span> <span style='color:#e74c3c'>de</span> <span style='color:#3498db'>couleur</span> <span style='color:#2ecc71'>sur</span> l'hexagone extérieur montrent le temps qu'il te reste pour enchaîner un combo.</p> <hr> <p id = 'afterhr'></p> Jeu créé par Logan Engstrom et Garrett Finucane (licence GPL-3, voir LICENSE.md). Copie traduite en français pour l'Espace détente.");
	if (gameState == 1) {
		pause();
	}
...
// RevisionApp (24/09/2026) : chargement d'un script distant (hextris.io/a.js) retiré.

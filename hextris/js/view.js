...
function drawScoreboard() {
	...
	if (gameState === 0) {
		...
		renderText(trueCanvas.width / 2 + gdx + 5 * settings.scale, h + 10, fontSize, "rgb(44,62,80)", 'Jouer !');
	} else if (gameState != 0 && textOpacity > 0) {
		textOpacity -= 0.05;
		renderText(trueCanvas.width / 2 + gdx + 6 * settings.scale, trueCanvas.height / 2 + gdy, 60, "rgb(236, 240, 241)", String.fromCharCode("0xf04b"), 'px FontAwesome');
		renderText(trueCanvas.width / 2 + gdx + 6 * settings.scale, trueCanvas.height / 2 + gdy - 155 * settings.scale, 150, "#2c3e50", "Hextris");
		renderText(trueCanvas.width / 2 + gdx + 5 * settings.scale, h, fontSize, "rgb(44,62,80)", 'Jouer !');
...
function showText(text) {
	var messages = {
		...
		'start': "<div class='centeredHeader unselectable' style='line-height:80px;'>Appuie sur Entrée pour commencer</div>"
	};
	// RevisionApp (24/09/2026) : messages de pause publicitaires (liens vers les
	// boutiques d'applications) retirés — toutes les pauses affichent « Pause ».
	if (text == 'pausedAndroid' || text == 'pausediOS' || text == 'pausedOther') text = 'paused';
